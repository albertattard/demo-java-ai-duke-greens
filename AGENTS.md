...
- Deliver one independently demonstrable vertical slice at a time. A slice must have one visitor-visible outcome that can be implemented, tested, reviewed, and committed without completing the wider user story. Keep the wider goal and any proposed sequence in `docs/current.md`, but make its “Current slice” section describe only the next increment. Do not begin a later increment until the current one has been verified and committed, unless the user explicitly directs otherwise.
- Prefer a thin behavioural increment over a complete story. For example, make a model clarification visible and resumable before adding broader conversation recovery, cleanup, or refinement behaviour. Do not add adjacent planned behaviour merely because it shares code or a user journey.
