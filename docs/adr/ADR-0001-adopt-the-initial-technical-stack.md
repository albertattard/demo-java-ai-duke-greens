...
| Application      | Spring Boot 4.1.0 modular monolith with Spring MVC            | Pins the current stable Spring Boot release for a reproducible initial baseline while providing the web application, dependency management, testing support, and a simple path to a single deployable application. [Spring release announcement](https://spring.io/blog/2026/06/10/spring-boot-4) |
...
| Browser security | Spring Security CSRF protection for state-changing form posts | Protects anonymous visitor-session state and prevents cross-site requests from triggering AI work or changing session state. Authentication and authorisation remain out of scope.                                                                                                                |
...
- Every state-changing browser form includes a CSRF token, and requests without a valid token are rejected.
...
- Browser forms gain CSRF-token rendering and verification, adding a small amount of template and test complexity while preventing unauthorised cross-site state changes and AI retries.
...
- Voice input/output, real-time inventory, MCP, RAG, curated recipes, payments, delivery, external ordering, authentication, authorisation, database migration tooling, durable persistence, a Node.js build pipeline, CDN dependencies, and client-side state-management remain outside this initial stack decision.
