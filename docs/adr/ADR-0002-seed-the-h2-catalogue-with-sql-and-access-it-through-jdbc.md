...
Represent prices as `BigDecimal` in the application-wide EUR price list. Do not persist currency on individual products: the MVP has no multi-currency catalogue or conversion policy. Render prices using the configured presentation locale, initially `Locale.GERMANY`.
