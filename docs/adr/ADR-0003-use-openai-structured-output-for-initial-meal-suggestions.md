...
TASK-0004 introduces Duke Greens' first live model interaction. A visitor submits one free-form request and receives one to seven meal suggestions. This first slice deliberately generates unconstrained meal ideas: it does not yet pass the catalogue to the model, map ingredients to products, show prices, or create a basket.
...
Use OpenAI through the Spring AI integration selected in [ADR-0001](ADR-0001-adopt-the-initial-technical-stack.md). Configure `gpt-5.4-mini` as the default model for TASK-0004. The model identifier is an application property and may be changed without code changes after evaluation.
...
- short explanation;
- servings; and
- one or more ingredients, each with a name, quantity, and unit.
...
Java owns the model-facing interface and response validation. It converts timeouts, provider failures, malformed structured output, and invalid response content into the visitor-facing error state defined by the MVP specification. It never presents partial suggestions or provider error details.
...
- Adding the OpenAI dependency and API-key configuration is implementation work in TASK-0004; neither is required for the deterministic automated tests.
