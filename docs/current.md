...
- Move basket contents, selected-meal controls, the basket total, fulfilment status, and reset controls from the recommendations page to a dedicated basket page.
...
- The basket’s product lines, whole-pack quantities, total, and fulfilment status are derived solely from the checked meals. Shared ingredient requirements are combined before package quantities are rounded.
...
- Selecting or clearing meal checkboxes and submitting “Update basket” updates the selected meals, derived product lines, quantities, total, fulfilment status, and recommendation-page basket summary without calling the model.
...
- All basket mutations redirect to the active basket or recommendations page and reject another session’s or stale conversation ID without reading or changing state.
